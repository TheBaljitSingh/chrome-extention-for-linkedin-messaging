// Function to create and append the SVG icon in the message input box
function createSvgInMessageInputBox() {
    // Check if the input box already has the SVG to avoid duplicates
    const existingIcon = document.querySelector('#my-custom-icon');
    if (existingIcon) {
        return; // Exit if the icon already exists
    }
    // Create the SVG icon element (you will need to replace this with your actual SVG code)
    const iconWrapper = document.createElement('div');
    const AiIcon = `<svg width="42" height="42" viewBox="0 0 42 42" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  <feBlend mode="normal" in="SourceGraphic" in2="effect2_dropShadow_3_37" result="shape"/>
    ...
    iconWrapper.id = 'my-custom-icon'; // Set an ID for reference
    iconWrapper.style.position = 'absolute';
    iconWrapper.style.top = '5px';
    iconWrapper.style.right = '5px';
    iconWrapper.style.cursor = 'pointer';
    iconWrapper.innerHTML = `${AiIcon}`;
    // Find the message input box and position the SVG inside it
    const messageInputBox = document.querySelector('.msg-form__contenteditable[contenteditable="true"]');
    if (messageInputBox) {
        //   messageInputBox.style.position = 'relative'; // Ensure the input box is positioned relatively
        messageInputBox.appendChild(iconWrapper);
    }
    // Add event listener to open the generate box when SVG is clicked
    iconWrapper.addEventListener('click', () => {
        openGenerateBox();
    });
}
// Function to open the generate box above the SVG icon
function openGenerateBox() {
    // Avoid creating multiple generate boxes if already present
    const existingBox = document.getElementById('my-generate-overlay');
    if (existingBox) {
        return; // Exit if the generate box already exists
    }

    // Create the overlay and input box
    const modalHtml = `
    ...
    // Add the modal overlay and box to the body
    const wrapper = document.createElement('div');
    wrapper.innerHTML = modalHtml;
    document.body.appendChild(wrapper);

    // Close the modal if clicking outside the box
    document.getElementById('my-generate-overlay').addEventListener('click', (event) => {
        if (event.target.id === 'my-generate-overlay') {
            closeGenerateBox();
        }
    });
    ...
    document.getElementById('generateButton').addEventListener('click', () => {
        ...
        // Call the dummy API to get generated text
    ...
    // Add event listener to the "Close" button to close the box
    document.getElementById('closeButton').addEventListener('click', () => {
        closeGenerateBox();
    });
}

// Function to simulate an API call and generate a response
function generateResponse(input) {
    return new Promise((resolve) => {
        setTimeout(() => {
            // Dummy response generation logic
            const response = `Thank you for the opportunity! If you have any more questions or if there's anything else I can help you with, feel free to ask regarding "${input}".`;
            resolve(response);
        }, 1000); // Simulating network delay
    });
}

// Function to update the modal with generated text
function updateModalWithGeneratedText(generatedText) {
    const modalBox = document.getElementById('my-generate-box');


    modalBox.innerHTML =
        `<p 
    style="margin: 0; color: #6B7280;">Generated Response:</p>
    ...
    document.getElementById('insertButton').addEventListener('click', () => {
        insertGeneratedText(generatedText); // Insert the generated text into the LinkedIn message box
        closeGenerateBox();
    });

    // Add event listener for "Regenerate" button
    document.getElementById('regenerateButton').addEventListener('click', () => {
        document.getElementById('userInput').value = ""; // Clear input for new query
        modalBox.innerHTML = `
            <input type="text" id="userInput" placeholder="Your prompt" style="padding: 10px; width: 100%; border: 1px solid #ccc; border-radius: 4px; margin-top: 10px;" />
            <div style="margin-top: 15px; text-align: right;">
        ...
        // Re-add event listeners for the new "Generate" and "Close" buttons
        document.getElementById('generateButton').addEventListener('click', () => {
            const userInput = document.getElementById('userInput').value;
            generateResponse(userInput)
                .then(generatedText => {
...
// Function to insert the generated text into the LinkedIn message area
function insertGeneratedText(text) {
    const messageArea = document.querySelector('.msg-form__contenteditable.t-14.t-black--light.t-normal.flex-grow-1.full-height.notranslate');
    const existingParagraph = messageArea.querySelector('p'); // Get the existing <p> tag

    if (existingParagraph) {
        // Set the inner HTML of the <p> tag with the generated text
        existingParagraph.innerHTML = text; // This will overwrite any existing content

        // Remove the placeholder if it's present
        if (existingParagraph.textContent === 'Your prompt' || existingParagraph.innerHTML.trim() === '<br>') {
            existingParagraph.innerHTML = ''; // Clear the content

        }
    } else {
        // If no <p> tag exists, create one and append it
        const p = document.createElement('p');
        p.innerHTML = text; // Set the new paragraph's inner HTML to the generated text
        messageArea.appendChild(p); // Append the new paragraph to the message area
        // messageArea.setAttribute('aria-label', ' '); // this is not working  solve it later - 
    }
}
...
// Function to close the generate box and remove the overlay
function closeGenerateBox() {
    const overlay = document.getElementById('my-generate-overlay');
    if (overlay) {
        overlay.remove(); // Directly remove the overlay from DOM
    }
}

// MutationObserver to detect changes and apply the SVG icon when the message input is loaded
const observer = new MutationObserver(() => {
    checkURLAndCreateSvg();
});
// Function to check URL and create the SVG icon
function checkURLAndCreateSvg() {
